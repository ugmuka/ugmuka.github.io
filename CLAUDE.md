...
`.pre-commit-config.yaml` で JSON の自動整形（`pretty-format-json`、2 スペース / キー名ソート、`name` は先頭固定）と構文チェック（`check-json`）を実行します。
...
除外対象: `package-lock.json`（npm 管理）、`tsconfig.app.json` / `tsconfig.node.json`（コメント付き JSONC のため）、`docs/`（ビルド成果物）。
